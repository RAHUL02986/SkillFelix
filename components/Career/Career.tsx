...
export default function Career() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("/api/sendInternshipForm", {
        method: "POST",
        body: form,
      });
      ...
      <div className="container w-full bg-white/70 backdrop-blur-md shadow-md rounded-xl flex flex-col md:flex-row items-center gap-10 py-10 px-4 lg:p-10">
        ...
        >
          <h2 className="text-3xl md:text-5xl font-normal mb-8 text-[#162726]">
            Internship opportunities at <span className="text-[#0B96F3]">SkillFelix</span>
          </h2>
          <p className="mb-4">Embark on a dynamic learning journey with SkillFelix...</p>
